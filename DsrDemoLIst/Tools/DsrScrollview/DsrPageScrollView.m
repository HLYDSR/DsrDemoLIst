...
@implementation DsrPageScrollView
...
-(void)addImgViewsWithImgUrls:(NSArray *)imgUrls withTitles:(NSArray *)titles{
    for (int i=0; i<imgUrls.count+2; i++) {
        ...
        if (self.pageViewType==DsrPageScrollViewFullScreenMode) {
...
-(void)imgTapAction:(UITapGestureRecognizer *)tap{
    [self.dsrPageScrollViewDelegate imgViewDidTouchActionAtIndex:self.pageControl.currentPage inArray:self.urls];
}
